...
class Seoul::Vga_vesa
{
	...
	public:
		...
		void init(VgaRegs *regs, char *guest_fb)
		{
			_regs = regs;
			_guest_fb = guest_fb;
		}
		...
		bool mode_info(MessageConsole &msg, Backend_gui &gui)
		{
			...
			if (msg.index == 0) {
				 ...
				 */
				unsigned const bpp = gui.fb_mode.bytes_per_pixel();
				msg.info->_vesa_mode = 0x138;
				msg.info->attr = 0x39f;
				msg.info->resolution[0]      = gui.fb_mode.area.w();
				msg.info->resolution[1]      = gui.fb_mode.area.h();
				msg.info->bytes_per_scanline = gui.fb_mode.area.w()*bpp;
				msg.info->bytes_scanline     = gui.fb_mode.area.w()*bpp;
				msg.info->bpp = bpp;
				...
				msg.info->_phys_size = gui.fb_mode.area.count()*bpp;
				return true;
			}
			return false;
		}
